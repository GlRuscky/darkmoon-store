...
<div class="container mt-5 mb-5">
    ...
    <div class="d-flex justify-content-center flex-wrap gap-2 mb-4">
        <a href="/darkmoon-store/produtos.php" class="btn btn-outline-light <?= empty($categoria) ? 'active' : '' ?>">Todos</a>
        <a href="/darkmoon-store/produtos.php?categoria=Camisas"    class="btn btn-outline-light <?= $categoria === 'Camisas'    ? 'active' : '' ?>">Camisas</a>
        <a href="/darkmoon-store/produtos.php?categoria=Vestidos"   class="btn btn-outline-light <?= $categoria === 'Vestidos'   ? 'active' : '' ?>">Vestidos</a>
        <a href="/darkmoon-store/produtos.php?categoria=Botas"      class="btn btn-outline-light <?= $categoria === 'Botas'      ? 'active' : '' ?>">Botas</a>
        <a href="/darkmoon-store/produtos.php?categoria=Acessorios" class="btn btn-outline-light <?= $categoria === 'Acessorios' ? 'active' : '' ?>">Acessórios</a>
    </div>

    <!-- Filtro de faixa de preço via formulário GET -->
    <form method="GET" action="/darkmoon-store/produtos.php" class="d-flex justify-content-center align-items-center flex-wrap gap-2 mb-4">
        <?php if (!empty($busca)): ?>
        ...
        <?php if (!empty($precoMin) || !empty($precoMax)): ?>
            <a href="/darkmoon-store/produtos.php?busca=<?= urlencode($busca) ?>&categoria=<?= urlencode($categoria) ?>" 
